...
Usage:
    python gen_bw_matrix.py -x 2 -y 2 -z 1 -o schedule.txt
...
def main():
    ...
    parser.add_argument(
        "-bw",
        "--bandwidth",
        help="Bandwidth (in GBps) for each link.",
        type=float,
        default=50.0,
    )
    parser.add_argument(
        "-x",
        "--x_dim",
        help="X dimension size of the topology (assuming torus).",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-y",
        "--y_dim",
        help="Y dimension size of the topology (assuming torus).",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-z",
        "--z_dim",
        help="Z dimension size of the topology (assuming torus).",
        type=int,
        default=1,
    )
